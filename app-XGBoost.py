...
if 'data' not in st.session_state:
    st.session_state['data'] = pd.DataFrame(
        columns=['Age(y)','PT (s)','CA199 (U/mL)','LDH (U/L)','CK (U/L)','Prediction','Label'])

# 在主页面上显示数据
st.header('30-day mortality of adult AP Patients based on XGboost Algorithm')
...
# Input bar 1
a = st.sidebar.number_input('Age(y)', min_value=0, max_value=100, value=28)
b = st.sidebar.number_input('PT (s)', value=None)
c = st.sidebar.number_input('CA199 (U/mL)',value=None)
d = st.sidebar.number_input('LDH (U/L)', value=None)
e = st.sidebar.number_input('CK (U/L)', value=None)
...
if st.sidebar.button("Submit"):
    ...
    new_data = pd.DataFrame([[X.iat[0, 0], X.iat[0, 1], X.iat[0, 2], X.iat[0, 3], X.iat[0, 4], result_prob_pos / 100, None]],
...
if uploaded_file is not None:
    ...
    # 检查是否所有必需的列都存在
    missing_cols = [col for col in ['Age','PT','CA199','LDH','CK'] if
    ...
    if missing_cols:
        ...
        for _, row in df.iterrows():
            ...
            new_data = pd.DataFrame([[X.iat[0, 0], X.iat[0, 1], X.iat[0, 2], X.iat[0, 3], X.iat[0, 4], result_prob, label]],
                                    columns=st.session_state['data'].columns)
            st.session_state['data'] = pd.concat([st.session_state['data'], new_data], ignore_index=True)
